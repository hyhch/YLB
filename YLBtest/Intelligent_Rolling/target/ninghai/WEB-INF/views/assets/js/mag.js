function mag(show, bigshow, smallshow, mask, bigitem,show1) { this.show = show;
  this.bigshow = bigshow;
  this.smallshow = smallshow;
  this.mask = mask;
  this.show1=show1;
  this.bigitem = bigitem;
  this.obj = { prev: '.prev', next: '.next', middle: '.middle', middleBox: '.middle_box' } };
mag.prototype = {
  init: function($) { var that = this;
    ...
    this.prevClick($);
    this.nextClick($); },
  start: function($) { var that = this; var buil = $(that.show).width() / $(that.mask).width() * $(that.bigshow).width();
    $(that.bigitem).css("width", buil);
    $(that.smallshow + ' img').eq(0).css("border", "2px solid #f40"); var midhei = $(that.obj.middle + ' li').innerWidth() * $(that.obj.middle + ' li').length;
    $(that.obj.middle).width(midhei); },
  showHover: function($) { var that = this;
    $(that.show).hover(function() { $(that.bigshow).show();
      $(that.mask).show(); }, function() { $(that.bigshow).hide();
      $(that.mask).hide(); }); },
  smallImgHover: function($) { var that = this;
    $(that.smallshow + ' img').click(function(event) { var src = $(this).attr("src");
      console.log($(this).attr("data_pty"));
      console.log($(event.currentTarget).attr('data-pty'));
      console.log(event);
      $(that.smallshow + ' img').css("border", "1px solid #e8e8e8");
      $(this).css("border", "2px solid #f40");
      //$(that.show1 + '>img').attr("src", src);
        $('#'+$(this).attr("data-pty") + '>img').attr("src", src);
  ...
  prevClick: function($) {
    ...
    $(that.obj.prev).click(function() {
      if ($(that.obj.middle).width() - $(that.obj.middleBox).width() > 0) {
        if (Math.abs(parseInt($(that.obj.middle).css("marginLeft"))) > $(that.obj.middleBox).width()) { $(that.obj.middle).css("marginLeft", parseInt($(that.obj.middle).css("marginLeft")) + $(that.obj.middleBox).width()) }
        if (Math.abs(parseInt($(that.obj.middle).css("marginLeft"))) < $(that.obj.middleBox).width()) { $(that.obj.middle).css("marginLeft", "0px");
